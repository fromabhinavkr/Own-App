...
public class GlobeGLSurfaceView extends GLSurfaceView {
    ...
    public GlobeGLSurfaceView(Context context, AttributeSet attrs) {
        ...
        // Standard tap and swipe detector (Pinch-to-zoom completely removed)
    ...
    public boolean onTouchEvent(MotionEvent e) {
        ...
        if (e.getAction() == MotionEvent.ACTION_MOVE) {
            ...
            // Lock the vertical rotation so it doesn't flip upside down
    ...
    private class GlobeRenderer implements GLSurfaceView.Renderer {
        ...
        public void onSurfaceChanged(GL10 unused, int width, int height) {
            ...
            // SMART SCALING: Dynamically adjust the camera distance based on the screen ratio!
            // This ensures the Earth takes up a consistent percentage of the screen WIDTH,
            // so it never looks tiny on long, narrow phones.
            float baseRatio = 0.5625f; // Standard 16:9 ratio
            cameraZ = -3.8f * (ratio / baseRatio);

            // Strict safety bounds so it never clips into a square and never gets too small
            if (cameraZ > -3.3f) cameraZ = -3.3f;
        ...
        public void onDrawFrame(GL10 unused) {
            ...
            Matrix.setLookAtM(viewMatrix, 0, 0, 0, cameraZ, 0f, 0f, 0f, 0f, 1.0f, 0.0f);
        ...
        public void changeTexture(int resourceId) {
            int oldTex = textureId;
            textureId = loadTexture(context, resourceId);
            if (oldTex != 0) {
                GLES20.glDeleteTextures(1, new int[]{oldTex}, 0);
            }
        }
