...
public class AudioGalleryActivity extends AppCompatActivity {
    private View galleryRoot; private TextView btnBack, tvGalleryTitle; private LinearLayout listContainer;
    private boolean isDarkTheme;

    // --- 3-State Theme Variables ---
    private int themeState;
    private int colorBg, colorCardBg, colorText, colorDialogBg, colorPillBg;
    ...
    @Override protected void onCreate(Bundle savedInstanceState) {
        ...
        if (themeState == 0) { // Light Mode
            colorBg = Color.WHITE;
            colorCardBg = Color.parseColor("#F2F2F7");
            ...
            colorCardBg = Color.parseColor("#2C2C2E");
            colorText = Color.WHITE;
            colorDialogBg = Color.parseColor("#CC1C1C1E");
            colorPillBg = Color.parseColor("#332D2B");
        } else { // Star Mode (AMOLED Pure Black)
            colorBg = Color.parseColor("#000000"); // Pure Black background
            colorCardBg = Color.parseColor("#1C1C1E"); // Elevated dark gray for cards
            colorText = Color.WHITE;
            colorDialogBg = Color.parseColor("#E61C1C1E"); // Translucent dialog
    ...
    private void applyTheme() {
        getWindow().setStatusBarColor(colorBg); galleryRoot.setBackgroundColor(colorBg); tvGalleryTitle.setTextColor(colorText);
    ...
    private void addFileToUI(File file, String subtitle) {
        int cardBg = colorCardBg, textColor = colorText;
    ...
    private void showRenameDialog(File file) {
        ...
        TextView title = new TextView(this); title.setText("Rename Track"); title.setTextColor(colorText); title.setTextSize(20f); title.setTypeface(null, Typeface.BOLD); title.setPadding(0, 0, 0, 60); title.setGravity(Gravity.CENTER); root.addView(title);
        EditText input = new EditText(this); String oldName = file.getName(); int dotIdx = oldName.lastIndexOf('.'); String ext = dotIdx > 0 ? oldName.substring(dotIdx) : ""; input.setText(dotIdx > 0 ? oldName.substring(0, dotIdx) : oldName); input.setTextColor(colorText); input.setPadding(40, 40, 40, 40); input.setBackground(createPill(colorPillBg)); root.addView(input);
