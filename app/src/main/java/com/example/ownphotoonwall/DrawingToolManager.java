...
public class DrawingToolManager {
    ...
    public static class DrawStroke {
        public Path path;
        public Paint paint;
        public int targetCanvasState; // 0 = draw, 1 = draw erase, 2 = bg erase, 3 = bg repair
        public DrawStroke(Path p, Paint pt, int tc) {
            path = new Path(p);
            paint = new Paint(pt);
            targetCanvasState = tc;
        }
    }

    private Bitmap drawLayerBitmap;
    private Canvas drawLayerCanvas;
    private Bitmap eraseLayerBitmap;
    private Canvas eraseLayerCanvas;
    ...
    public DrawingToolManager() {
        currentDrawPaint.setStyle(Paint.Style.STROKE);
        currentDrawPaint.setStrokeJoin(Paint.Join.ROUND);
        currentDrawPaint.setStrokeCap(Paint.Cap.ROUND);
    }

    public void initBitmaps(int width, int height) {
        if (width <= 0 || height <= 0) return;
        drawLayerBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
        drawLayerCanvas = new Canvas(drawLayerBitmap);
        eraseLayerBitmap = Bitmap.createBitmap(width, height, Bitmap.Config.ARGB_8888);
    ...
    public void configurePaint(float scaleFactor, boolean isBgRemoverMode, boolean isBgRepairMode) {
        ...
        if (isDrawMode && drawLayerCanvas != null) {
            if (isDrawEraserMode) {
                currentDrawPaint.setXfermode(new PorterDuffXfermode(PorterDuff.Mode.CLEAR));
                currentDrawPaint.setColor(Color.TRANSPARENT);
            } else {
                currentDrawPaint.setXfermode(null);
                currentDrawPaint.setColor(currentBrushColor);
                currentDrawPaint.setAlpha(currentBrushOpacity);
            }
            ...
            if (isBgRepairMode) {
                ...
                currentDrawPaint.setXfermode(null);
                currentDrawPaint.setColor(Color.BLACK);
            }
        }
    }
